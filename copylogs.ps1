...
foreach ( $server in $servers ) 
{
    ...
    $password = ConvertTo-SecureString  '3w"+:aEm;pVq=Jy@VNmMR-`:A1rDUl#^'-AsPlainText -Force

    $Cred = Get-Credential 
    Copy-Item -Path $sourcepath -Destination $destination -Include $include -Recurse -Credential 
